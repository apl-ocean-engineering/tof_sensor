...
ExtrinsicCalibration::ExtrinsicCalibration(std::string image_topic) : it_(nh_) {
  ...
  **********************************************************/
  ...
  image_sub_ =
      it_.subscribe(image_topic, 1, &ExtrinsicCalibration::imageCb, this);
...
void ExtrinsicCalibration::img_info_callback(
...
void ExtrinsicCalibration::pointcloud_callback(
...
std::vector<Point2f>
ExtrinsicCalibration::matrix_to_vector(const Eigen::MatrixXd X) {
  /**************************************************
...
ExtrinsicCalibration::chess_board_points() {
  ...
  ********************************************************/
  std::vector<cv::Point2f> checkerboard_points_vector; // Vector to record
                                                       // points
  cv::Mat imgGray;
  ...
  if (found) {
    ...
    for (int i = 0; i < patternsize.height; i++) {
      for (int j = 0; j < patternsize.width; j++) {
        ...
        // patternsize.height * checkerboardDist - (i + 1) * checkerboardDist;
...
std::tuple<Mat, Mat> ExtrinsicCalibration::transformation_from_homography(
  ...
  ********************************************************************/
  ...
  std::cout << "R selected" << R << std::endl << std::endl;
  std::cout << "T selected" << T << std::endl << std::endl;
  return std::make_tuple(R, T);
}

std::tuple<bool, Eigen::Matrix4d>
ExtrinsicCalibration::planar_transformation(const std::vector<Point2f> src_vec,
                                            const std::vector<Point2f> dst_vec,
                                            const int board) {
  /*************************************************************
  ...
  *************************************************************/
  ...
  if (rotations.size() == 4) {
    std::tuple<Mat, Mat> transformation =
        transformation_from_homography(rotations, translations, normals, board);
...
void ExtrinsicCalibration::static_frame_transformation() {
  /*******************************************************************************
  Determine a scaled rotation and translation between frames from one static
  frame
  *******************************************************************************/
  ...
  Eigen::MatrixXd Xi =
      std::get<1>(chess_points); // Image plane homogenous points
  Eigen::MatrixXd Xw =
      std::get<2>(chess_points); // World plane homogenous points
  ...
  ***********************************/
  ...
  std::cout << "camera transformation: " << std::endl
            << color_camera_transformation << std::endl
            << std::endl;
...
ExtrinsicCalibration::calculate_extrinsic_paramaters() {
  ...
  *******************************************************************************/
  ...
  if (camera_transformation_vector.size() !=
      depth_transformation_vector.size()) {
...
void ExtrinsicCalibration::run() {
  ...
  // Terminal display
  ROS_INFO(
      "CLICK POINTS ON BOARD. PRESS 's' TO RECORD POINTS FOR 1 STATIC FRAME");
...
int main(int argc, char **argv) {
  ...
  if (n.getParam("image_info", camera_info)) {
    ...
    ROS_ERROR(
        "No info topic specified, subscribing to /camera/rgb/camera_info");
  ...
  if (n.getParam("image", image_topic)) {
    ...
    ROS_ERROR(
        "No image topic specified, subscribing to  /camera/rgb/image_color");
  ...
  if (n.getParam("points", pointcloud)) {
    ...
    ROS_ERROR(
        "No pointcloud topic specified, subscribed to /camera/depth/points");
  ...
  ros::Subscriber info_sub = n.subscribe(
      camera_info, 1000, &ExtrinsicCalibration::img_info_callback, &ec);
  ros::Subscriber pointcloud_sub = n.subscribe(
      pointcloud, 1000, &ExtrinsicCalibration::pointcloud_callback, &ec);
